...
import lgd.asset.metrics_ogb as metrics_ogb
from lgd.asset.metric_wrapper import MetricWrapper
...
class CustomLogger(Logger):
    ...
    def update_stats(self, true, pred, loss, lr, time_used, params,
        ...
        if dataset_name == 'ogbg-code2':
            ...
            from ldg.loader.ogbg_code2_utils import idx2vocab, \
                decode_arr_to_seq
    ...
    def metric_stat(self, metric_best):
        if self.task_type == 'regression':
            task_stats = self.regression()
        elif self.task_type == 'classification_binary':
            task_stats = self.classification_binary()
