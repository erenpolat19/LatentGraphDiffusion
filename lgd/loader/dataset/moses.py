...
from lgd.asset.molecules_evaluation import mol2smiles, build_molecule_with_partial_charges, compute_molecular_metrics
...
class MOSESDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for i, smile in enumerate(tqdm(smiles_list)):
            ...
            x = torch.tensor(type_idx, dtype=torch.long).unsqueeze(1)
            ...
            if self.filter_dataset:
                ...
                mol = build_molecule_with_partial_charges(data.x, adj, atom_decoder)
...
def get_moses_smiles(cfg, dataloader=None, dataset_infos=None, evaluate_dataset=False):
    ...
    # logging.info(smiles_path)

    smiles_path = 'datasets/MOSES/smiles.npy'
    ...
    if os.path.exists(smiles_path):
        ...
        train_smiles = compute_smiles(dataloader)
...
def compute_smiles(dataloader):
    print(f"\tConverting MOSES dataset to SMILES...")
    ...
    for i, batch in enumerate(dataloader):
        ...
        for b in range(B):
            ...
            mol = build_molecule_with_partial_charges(X, E.reshape(X.shape[0], X.shape[0]), atom_decoder)
        ...
        if i % 1000 == 0:
            print("\tConverting MOSES dataset to SMILES {0:.2%}".format(float(i) / len_train))
