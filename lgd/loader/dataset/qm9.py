    ...
    def process(self):
        ...
        for i, mol in enumerate(tqdm(suppl)):
            ...
            if self.remove_h:
                ...
                x = x[to_keep]
                # Shift encoding to match atom decoder
                x = x - 1
...
class QM9infos(AbstractDatasetInfos):
    def __init__(self, cfg, datamodule=None, recompute_statistics=False):
        self.remove_h = cfg.dataset.remove_h
...
def get_qm9_smiles(cfg, train_dataloader, dataset_infos, evaluate_dataset=False, test_set=False):
    if evaluate_dataset:
        assert dataset_infos is not None, "If wanting to evaluate dataset, need to pass dataset_infos"
    # datadir = cfg.dataset.datadir
    datadir = 'datasets/QM9'
