...
def init_model_from_pretrained(model, pretrained_dir,
    ...
    if reset_prediction_head:
        ...
        pretrained_dict = {k: v for k, v in pretrained_dict.items()
                           if (not k.startswith('post_mp') and not k.startswith('first_stage_model.decode')
                               and not k.startswith('cond_stage_model.decode'))}
    # Overwrite entries in the existing state dict.
    model_dict.update(pretrained_dict)
    # Load the new state dict.
    missing, unexpected = model.load_state_dict(model_dict, strict=False)
    logging.info(f"Restored from {ckpt_file} with {len(missing)} missing and {len(unexpected)} unexpected keys")
    if len(missing) > 0:
    ...
    if freeze_main:
        for key, param in model.named_parameters():
            if (not key.startswith('post_mp') and not key.startswith('first_stage_model.decode')):
                param.requires_grad = False
            elif reset_prediction_head:
                param.requires_grad = True
    elif cfg.pretrained.get("train_diffusion_only", False):
        for key, param in model.named_parameters():
