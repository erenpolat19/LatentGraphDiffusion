...
class SANGraphHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, L=2):
        super().__init__()
        self.pooling_fun = register.pooling_dict[cfg.encoder.pool]
        list_FC_layers = [
        ...
        self.L = L
        self.activation = register.act_dict[cfg.encoder.act]()

    def _apply_index(self, batch):
        return batch.graph_attr

    def forward(self, batch, **kwargs):
        if 'task' in kwargs and kwargs['task'] == 'graph':
            graph_emb = batch.graph_attr
        else:
