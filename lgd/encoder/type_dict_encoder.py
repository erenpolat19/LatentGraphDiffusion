...
class TypeDictNodeEncoder(torch.nn.Module):
    def __init__(self, emb_dim, node_encoder_num_types=None):
        super().__init__()

        num_types = node_encoder_num_types if node_encoder_num_types is not None else cfg.dataset.node_encoder_num_types
        if cfg.dataset.get('add_virtual_node_edge', False):
            num_types += 6
        if num_types < 1:
...
class TypeDictEdgeEncoder(torch.nn.Module):
    def __init__(self, emb_dim, edge_encoder_num_types=None):
        super().__init__()

        num_types = edge_encoder_num_types if edge_encoder_num_types is not None else cfg.dataset.edge_encoder_num_types
        if cfg.dataset.get('add_virtual_node_edge', False):
            num_types += 10
        if num_types < 1:
    ...
    def forward(self, edge_attr):
        if isinstance(edge_attr, torch.Tensor):
            ...
            edge_attr.edge_attr = self.encoder(edge_attr.edge_attr)
        return edge_attr
