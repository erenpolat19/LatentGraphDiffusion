...
def pretrain_epoch(logger, loader, model, optimizer, scheduler, batch_accumulation, visualize=False):
    ...
    for iter, batch in enumerate(loader):
        ...
        if hasattr(model.model, 'decode_recon') and callable(model.model.decode_recon):
            edge_recon, edge_recon_tuple = model.model.decode_recon(pred)
            loss_structure_recon = criterion_edge(edge_recon, edge_label) * cfg.train.pretrain.edge_factor \
                                 + criterion_edge(edge_recon_tuple, edge_label)
        else:
            loss_structure_recon = 0
        loss = loss_edge * cfg.train.pretrain.edge_factor
        ...
        if cfg.gnn.head == 'inductive_edge':
            ...
            if ensemble_mode == 'none':
                ...
                if cfg.train.pretrain.recon != 'none':
                    ...
                    loss_recon = loss_edge * cfg.train.pretrain.edge_factor
                    if hasattr(model.model, 'decode_recon') and callable(model.model.decode_recon):
                        edge_recon, tuple_recon = model.model.decode_recon(pred)
                        loss_structure_recon_edge = criterion_edge(edge_recon, edge_label)
                        loss_structure_recon_tuple = criterion_edge(tuple_recon, edge_label)
                    else:
...
@register_train('pretrain_encoder_synthetic')
