...
def train_epoch(logger, loader, model, optimizer, scheduler, batch_accumulation):
    ...
    for iter, batch in enumerate(loader):
        ...
        batch.edge_attr_masked = batch.edge_attr.clone().detach() if batch.get('edge_attr', None) is not None else None
        loss, loss_task, pred, loss_node, loss_edge, loss_graph = model.training_step(batch)
    ...
    for batch in loader:
        ...
        if cfg.gnn.head == 'inductive_edge':
            ...
            if ensemble_mode == 'none':
                ...
                loss_task, graph_pred = model.inference(batch, ddim_steps=ddim_steps, ddim_eta=ddim_eta, use_ddpm_steps=use_ddpm_steps)
                ...
                for i in range(repeat):
                    ...
                    bc.edge_attr_masked = batch.edge_attr.clone().detach() if batch.get('edge_attr', None) is not None else None
                    # loss_generation, loss_graph, graph_pred = model.validation_step(bc)
                    loss_task, graph_pred = model.inference(bc, ddim_steps=ddim_steps, ddim_eta=ddim_eta, use_ddpm_steps=use_ddpm_steps)
        ...
        if cfg.dataset.name == 'ogbg-code2':
            ...
            if cfg.dataset.format in ['PyG-Planetoid', 'PyG-Amazon', 'PyG-Coauthor'] or cfg.dataset.transductive:
                true = batch.y.clone().detach()[batch.get(batch.split + '_mask', None)].flatten()
            else:
        ...
        logger.update_stats(true=_true,
                            pred=_pred,
                            loss=loss_task.detach().cpu().item(),
...
@register_train('train_diffusion_inductive')
