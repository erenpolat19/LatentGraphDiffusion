...
def train_epoch(logger, loader, model, prefix, optimizer, optimizer_prefix, scheduler, batch_accumulation):
    model.train()
    prefix.train()
    optimizer.zero_grad()
    ...
    for iter, batch in enumerate(loader):
        ...
        batch.prefix = prefix.prefix
        loss, loss_graph, graph_pred = model.training_step(batch)
        criterion_graph = nn.L1Loss()
        loss_graph = criterion_graph(graph_pred, graph_label)
        loss = loss + loss_graph * cfg.diffusion.graph_factor
        ...
        if ((iter + 1) % batch_accumulation == 0) or (iter + 1 == len(loader)):
            ...
            if cfg.pretrained.reset_prediction_head or not cfg.pretrained.freeze_main:
                optimizer.step()
            optimizer.zero_grad()
            optimizer_prefix.zero_grad()
...
@torch.no_grad()
def eval_epoch(logger, loader, model, prefix, split='val', repeat=1, ensemble_mode='none'):
    model.eval()
    prefix.eval()
    time_start = time.time()
    for batch in loader:
        ...
        logger.update_stats(true=_true,
                            pred=_pred,
                            loss=loss_generation.detach().cpu().item(),
...
@register_train('prefix_tuning_diffusion')
def custom_train_diffusion(loggers, loaders, model, prefix, optimizer, optimizer_prefix, scheduler):
    """
    ...
    for cur_epoch in range(start_epoch, cfg.optim.max_epoch):
        ...
        train_epoch(loggers[0], loaders[0], model, prefix, optimizer, optimizer_prefix, scheduler,
        ...
        if cur_epoch > cfg.train.start_eval_epoch:
            if is_eval_epoch(cur_epoch):
                for i in range(1, num_splits):
                    eval_epoch(loggers[i], loaders[i], model, prefix,
