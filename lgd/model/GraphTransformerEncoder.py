...
#         self.bn_no_runner = cfg.bn_no_runner
#
#         self.node_dict_dim = cfg.node_encoder_num_types + 10  # TODO: update this dimension of dictionary
#         self.edge_dict_dim = cfg.edge_encoder_num_types + 10
#         self.node_emb = nn.Embedding(self.node_dict_dim, self.in_dim, padding_idx=0)
#         self.edge_emb = nn.Embedding(self.edge_dict_dim, self.in_dim, padding_idx=0)
#         # NodeEncoder = register.node_encoder_dict[cfg.node_encoder_name]
#         # self.node_emb = NodeEncoder(self.in_dim)
...
#         self.prefix_emb = nn.Embedding(self.num_tasks, self.prefix_dim, padding_idx=0)  # TODO: whether need this padding_idx=0?
#
#         self.label_embed = nn.Sequential(nn.Linear(1, 2 * self.hid_dim), self.act,
#                                          nn.Linear(2 * self.hid_dim, self.hid_dim), nn.LayerNorm(self.hid_dim), self.act)
#
#         if self.posenc_in_dim > 0 and self.posenc_dim > 0:
#             self.posenc_emb = nn.Sequential(nn.Linear(self.posenc_in_dim, 2 * self.posenc_dim), self.act,
#                                             nn.Linear(2 * self.posenc_dim, self.posenc_dim), nn.LayerNorm(self.posenc_dim))
#
#         self.node_in_mlp = nn.Sequential(nn.Linear(self.hid_dim, 2 * self.hid_dim), self.act,
#                                          nn.Linear(2 * self.hid_dim, self.hid_dim))
#         self.edge_in_mlp = nn.Sequential(nn.Linear(self.hid_dim, 2 * self.hid_dim), self.act,
...
#                                          O_e=cfg.O_e,
#                                          ff_e=cfg.ff_e,
#                                          cfg=cfg) for _ in range(self.num_layers - 1)
#         ])
#
#         self.final_layer = GraphTransformerEncoderLayer(in_dim=self.hid_dim,
#                                                         out_dim=self.hid_dim,
...
#             self.graph_out_mlp = nn.Sequential(nn.Linear(self.out_dim, 2 * self.out_dim), self.act,
#                                                nn.Linear(2 * self.out_dim, self.out_dim))
#         self.decode_node = nn.Linear(self.out_dim, cfg.node_encoder_num_types + 2)  # virtual node
#         self.decode_edge = nn.Linear(self.out_dim, cfg.edge_encoder_num_types + 5)  # no edge, virtual edge, loop
#         self.decode_graph = nn.Linear(self.out_dim, 1)
#
#     def forward(self, batch, t=None, prefix=None, label=None):
#         # batch.x_0 = batch.x
#         # batch.edge_attr_0 = batch.edge_attr
#         num_nodes, num_edges = batch.num_nodes, batch.edge_index.shape[1]
...
#
#         if self.posenc_dim > 0:
#             if self.posenc_in_dim > 0 and batch.get("posenc", None) is not None:
#                 batch_posenc_emb = self.posenc_emb(batch.posenc)
#             else:
#                 batch_posenc_emb = torch.zeros([num_nodes, self.posenc_dim], dtype=torch.float, device=batch.x.device)
#             batch_posenc_edge = torch.zeros([num_edges, self.posenc_dim])
#             # h = torch.cat([h, prefix_h, batch_posenc_emb], dim=1)
#             # e = torch.cat([e, prefix_e, batch_posenc_edge], dim=1)
#             h = torch.cat([h, batch_posenc_emb], dim=1)
#             e = torch.cat([e, batch_posenc_edge], dim=1)
#         # else:
#         #     h = torch.cat([h, prefix_h], dim=1)
#         #     e = torch.cat([e, prefix_e], dim=1)
#
...
#             batch = self.GTE_layers[_](batch, temb)
#
#         batch = self.final_layer(batch, temb)
#
#         v_g = batch.x[virtual_node_idx]
#         if self.pool != 'none':
#             v_g = v_g + self.graph_out_mlp(self.global_pool(batch.x, batch_node_idx))
#             # TODO: do we need to change the virtual nodes in batch.x? used for classification loss
#             # batch.x[virtual_node_idx] = v_g
#         batch.graph_attr = v_g
#         return batch
#
#     def encode(self, batch, t=None, prefix=None, label=None):
#         return self.forward(batch, t, prefix, label)
#
...
# @register_network('GraphTransformerEncoder_Concat')
# class GraphTransformerEncoder_Concat(nn.Module):
#     """
#         Full Graph Transformer Encoder
#     """
#     def __init__(self, dim_in=0, dim_out=0, # in_dim, posenc_in_dim, posenc_dim, prefix_dim, hid_dim, out_dim,
...
#         self.bn_no_runner = cfg.bn_no_runner
#
#         self.node_dict_dim = cfg.node_encoder_num_types + 3  # TODO: update this dimension of dictionary
#         self.edge_dict_dim = cfg.edge_encoder_num_types + 3
#         self.node_emb = nn.Embedding(self.node_dict_dim, self.in_dim)
#         self.edge_emb = nn.Embedding(self.edge_dict_dim, self.in_dim, padding_idx=0)
#         # NodeEncoder = register.node_encoder_dict[cfg.node_encoder_name]
#         # self.node_emb = NodeEncoder(self.in_dim)
#         # EdgeEncoder = register.edge_encoder_dict[cfg.edge_encoder_name]
#         # self.edge_emb = EdgeEncoder(self.in_dim)
#
#         self.num_tasks = cfg.get("num_tasks", 19)  # 19 for qm9
#         self.prefix_type = cfg.get("prefix_type", "add_all")
#         assert self.prefix_type in ["add_virtual", "add_all", "concat"]
#         if self.prefix_type == "concat":
#             assert cfg.in_dim + cfg.prefix_dim + cfg.posenc_dim == cfg.hid_dim
#         else:
...
#             self.graph_out_mlp = nn.Sequential(nn.Linear(self.out_dim, 2 * self.out_dim), self.act,
#                                                nn.Linear(2 * self.out_dim, self.out_dim))
#         self.decode_node = nn.Linear(self.out_dim, cfg.node_encoder_num_types)  # 5 for qm9
#         self.decode_edge = nn.Linear(self.out_dim, cfg.edge_encoder_num_types)  # 5 for qm9 including no edge
#         self.decode_graph = nn.Linear(self.out_dim, 1)
#
#     def forward(self, batch, t=None, prefix=None, label=None):
#         # batch.x_0 = batch.x
...
#         if self.posenc_dim > 0:
#             if self.posenc_in_dim > 0 and batch.get("posenc", None) is not None:
#                 batch_posenc_emb = self.posenc_emb(batch.posec)
#             else:
#                 batch_posenc_emb = torch.zeros([num_nodes, self.posenc_dim], dtype=torch.float, device=batch.x.device)
#             batch_posenc_node = batch_posenc_emb[batch_node_idx]
#             batch_posenc_edge = batch_posenc_emb[batch_edge_idx]
#             h = torch.cat([h, batch_posenc_node], dim=1)
#             e = torch.cat([e, batch_posenc_edge], dim=1)
#
#         if label is not None:
#             if len(label.shape) == 1:
...
#             label_embed_node = label_embedding[batch_node_idx]
#             label_embed_edge = label_embedding[batch_edge_idx]
#             h = h + label_embed_node
#             e = e + label_embed_edge
#
#         if batch.get("prefix", None) is not None:
